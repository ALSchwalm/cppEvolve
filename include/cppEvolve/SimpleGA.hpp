...
/// Function which will return instances of Genome to be used in the initial
/// population
template <typename Genome>
using GeneratorType = std::function<Genome()>;

/// Function which returns a double representing the fitness of a member of the
/// population
template <typename Genome>
using EvaluatorType = std::function<double(const Genome&)>;

/// Function which returns a new member of the population by combining two
/// parents
template <typename Genome>
using CrossoverType = std::function<Genome(const Genome&, const Genome&)>;
...
template <typename Genome>
using SelectorType =
    std::function<void(std::vector<Genome>&, EvaluatorType<Genome>)>;

/*!
 * Defines a Simple genetic algorithm. That is, an algorithm for which no
 * special behavior is required to copy or modify the genome.
 */
template <typename Genome, size_t PopSize = 100>
class SimpleGA {
public:
    /*!
     * @param _generator A function which will return instances of Genome to be
     * used in the initial population
     *
     * @param _evaluator A function which returns a double representing the
     * fitness of a member of the population
     *
     * @param _crossover A function which returns a new member of the population
     * by combining two parents
     *
     * @param _mutator A function which will alter a member of the population in
     * some way
     *
     * @param _selector A function which removes the less fit members from the
     * current population
     */
    SimpleGA(GeneratorType<Genome> _generator, EvaluatorType<Genome> _evaluator,
             CrossoverType<Genome> _crossover, MutatorType<Genome> _mutator,
